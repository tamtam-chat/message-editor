...
class ParserState {
    ...
    flushText() {
        if (this.hasPendingText()) {
            ...
            const token = {
                type: "text" /* Text */,
                format: exports.TokenFormat.None,
...
const charToFormat = new Map([
    [42 /* Asterisk */, exports.TokenFormat.Bold],
    [95 /* Underscore */, exports.TokenFormat.Italic],
    [126 /* Tilde */, exports.TokenFormat.Strike],
    [96 /* BackTick */, exports.TokenFormat.Monospace],
]);
...
function formatForChar(ch) {
    return charToFormat.get(ch) || exports.TokenFormat.None;
}
...
function isStartBound(state) {
    ...
    if ((token === null || token === void 0 ? void 0 : token.type) === "markdown" /* Markdown */ && (token.format & exports.TokenFormat.LinkLabel)) {
        return true;
    }
    return false;
}
...
function consumeOpen(state) {
    ...
    while (state.hasNext()) {
        ...
        if (nextFormat !== exports.TokenFormat.None && !state.hasFormat(nextFormat)) {
            state.pos++;
...
function consumeClose(state) {
    ...
    while (state.hasNext()) {
        nextFormat = formatForChar(state.peek());
        if (nextFormat !== exports.TokenFormat.None && (format & nextFormat)) {
            state.pos++;
            format &= ~nextFormat;
...
function customLink(state) {
    ...
    if (state.consume(91 /* SquareBracketOpen */)) {
        pushOpen(state, mdToken(state, exports.TokenFormat.LinkLabel));
        return true;
    }
    if (state.consume(93 /* SquareBracketClose */) && (state.format & exports.TokenFormat.LinkLabel)) {
        // Нашли закрывающий токен ссылки: он имеет смысл только в том случае,
        // если за ним сразу следует ссылка в виде `(mail.ru)`
        const closeLabel = mdToken(state, exports.TokenFormat.LinkLabel);
        if (state.consume(40 /* RoundBracketOpen */)) {
            const openLink = mdToken(state, exports.TokenFormat.Link);
            const start = state.pos;
            if (consumeCustomLinkClose(state)) {
                ...
                state.pos++;
                pushClose(state, mdToken(state, exports.TokenFormat.Link));
                return true;
            }
        }
    }
...
function parse$1(text, opt) {
    ...
    if (options.markdown && state.formatStack.length) {
        ...
        for (let i = 0, token; i < state.formatStack.length; i++) {
            ...
            token.format = exports.TokenFormat.None;
...
const formats$1 = [
    [exports.TokenFormat.Bold, 'bold'],
    [exports.TokenFormat.Italic, 'italic'],
    [exports.TokenFormat.Monospace, 'monospace'],
    [exports.TokenFormat.Strike, 'strike'],
    [exports.TokenFormat.Underline, 'underline'],
    [exports.TokenFormat.Heading, 'heading'],
    [exports.TokenFormat.Marked, 'marked'],
    [exports.TokenFormat.Highlight, 'highlight'],
    [exports.TokenFormat.Link, 'md-link'],
    [exports.TokenFormat.LinkLabel, 'md-link-label'],
];
...
function renderText(token, elem, options) {
    ...
    if (emoji && options.emoji) {
        let offset = 0;
        const state = new ReconcileState(elem, options);
        if (!options.replaceTextEmoji || (token.format & exports.TokenFormat.Monospace)) {
...
function getTokenTypeClass(token) {
    if (isAutoLink(token) && (token.format & exports.TokenFormat.Monospace)) {
        return '';
    }
...
function isRenderLink(token) {
    if ((token.format & exports.TokenFormat.Monospace)) {
...
function mdToText(tokens, range) {
    ...
    const result = [];
    let format = exports.TokenFormat.None;
    for (let i = 0, token, len; i < tokens.length; i++) {
        token = tokens[i];
        len = token.value.length;
        if (token.type === "markdown" /* Markdown */) {
            if (token.format & exports.TokenFormat.LinkLabel) {
...
function textToMdToken(token, state) {
    let link = '';
    let { format } = token;
    let linkChange = exports.TokenFormat.None;
    let hasLink = 0;
    let bound = 0;
    let hasBound = false;
    let suffix;
    if (isCustomLink(token)) {
        format |= exports.TokenFormat.Link;
        link = token.link;
        if (state.link && token.link !== state.link) {
            ...
            // и снова открылась
            linkChange = exports.TokenFormat.Link;
        }
    }
    ...
    if (removed) {
        ...
        hasLink = removed & exports.TokenFormat.Link;
    ...
    if (added) {
        ...
        hasLink = added & exports.TokenFormat.Link;
        if (hasLink) {
            bound = 0;
            state.link = link;
        }
...
function getFormatSymbols(format, state, close) {
    ...
    if (close) {
        ...
        if ((format & exports.TokenFormat.Link) && state.link) {
            result += `](${state.link})`;
            state.link = '';
        }
    }
    else {
        if (format & exports.TokenFormat.Link) {
            result += '[';
        }
    ...
    if (customLink.length) {
        ...
        customLink.slice(0, -4).forEach(token => {
            if (token.type === "markdown" /* Markdown */) {
                ...
                output.push({
                    type: "link" /* Link */,
                    format: token.format & (~exports.TokenFormat.LinkLabel),
                    value: token.value,
                    auto: false,
                    emoji: token.emoji,
                    link,
...
function findCustomLinkBound(tokens, start) {
    ...
    while (start < tokens.length) {
        token = tokens[start++];
        if (token.type === "markdown" /* Markdown */ && (token.format & exports.TokenFormat.Link)) {
            if (linkFound) {
                return start;
            }
            linkFound = true;
...
class Editor {
    ...
    insertText(pos, text) {
        ...
        if (this.options.resetFormatOnNewline && !this.isMarkdown && /^[\n\r]+$/.test(text)) {
            updated = setFormat(updated, exports.TokenFormat.None, pos, text.length);
        }
...
exports.mdToText = mdToText;
exports.parse = parse$1;
exports.rangeToLocation = rangeToLocation;
exports.render = render;
exports.setFormat = setFormat;
exports.setLink = setLink;
exports.slice = slice;
exports.split = split;
exports.textToMd = textToMd;
exports.tokenForPos = tokenForPos;
